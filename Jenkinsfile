            ...
            steps {
                script {
                    withCredentials([usernamePassword(credentialsId: 'AndreiSVN', usernameVariable: 'github_user', passwordVariable: 'github_password')]) {
                        bat 'echo START'
                        bat 'cd runner-script'
                        bat 'call npm install'
                        bat 'call npm run build'
                        bat 'call npm run start-nunit "${params.testsToRunConverted}" "${params.runnerJarPath}" "${params.nunit2}" "${params.nunit3}" ${suiteId} ${suiteRunId} $github_user $github_password'
